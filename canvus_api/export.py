...
class WidgetImporter:
    ...
    async def _import_widget(
        ...
        try:
            if widget_type.lower() == "note":
                ...
                # Images require file upload via /images endpoint - cannot use /widgets
                # Check if we have an exported asset file to upload
                asset_file = self._find_asset_file(export_folder, original_id, "image")
                if asset_file:
                    new_widget = await self.client.create_image(
                        target_canvas, str(asset_file), import_data
                    )
                else:
                    print(
                        f"⚠️ Skipping Image widget {original_id} - requires file upload via /images endpoint"
                    )
                    return {
                        ...
                        "widget_type": widget_type,
                        "status": "skipped",
                        "reason": "File-based widget requires asset file for upload",
                    }
            elif widget_type.lower() == "video":
                # Videos require file upload via /videos endpoint - cannot use /widgets
                asset_file = self._find_asset_file(export_folder, original_id, "video")
                if asset_file:
                    new_widget = await self.client.create_video(
                        target_canvas, str(asset_file), import_data
                    )
                else:
                    print(
                        f"⚠️ Skipping Video widget {original_id} - requires file upload via /videos endpoint"
                    )
                    return {
                        "original_id": original_id,
                        "new_id": None,
                ...
                # PDFs require file upload via /pdfs endpoint - cannot use /widgets
                asset_file = self._find_asset_file(export_folder, original_id, "pdf")
                if asset_file:
                    new_widget = await self.client.create_pdf(
                        target_canvas, str(asset_file), import_data
                    )
                else:
                    print(
                        f"⚠️ Skipping PDF widget {original_id} - requires file upload via /pdfs endpoint"
                    )
                    return {
                        "original_id": original_id,
                        "new_id": None,
    ...
    def _find_asset_file(
        self, export_folder: Path, widget_id: str, widget_type: str
    ) -> Optional[Path]:
        """Find an exported asset file for a widget.

        Args:
            export_folder: Path to the export folder
            widget_id: ID of the widget
            widget_type: Type of widget (image, video, pdf)

        Returns:
            Path to the asset file if found, None otherwise
        """
        assets_dir = export_folder / "assets"
        if not assets_dir.exists():
            return None

        # Common extensions by widget type
        extensions = {
        ...
        type_extensions = extensions.get(widget_type.lower(), [])

        # Look for files matching widget_id with appropriate extension
        for ext in type_extensions:
            # Try direct match
            asset_path = assets_dir / f"{widget_id}{ext}"
            if asset_path.exists():
                return asset_path

            # Try with common naming patterns
            for pattern in [f"{widget_id}_*{ext}", f"*_{widget_id}{ext}"]:
