...
def widget_bounding_box(
    widget: Union[
        BaseWidget, Widget, Note, Image, Browser, Video, PDF, Anchor, Connector
    ],
) -> Rectangle:
    """
    ...
    if isinstance(widget, Connector):
        ...
        if (
            "x" not in src_location
            or "y" not in src_location
            or "x" not in dst_location
            or "y" not in dst_location
        ):
...
def distance_between_widgets(
    ...
    return (
        ...
        else max(horizontal_distance, vertical_distance)
    )


def find_widgets_in_area(
    widgets: List[
        Union[BaseWidget, Widget, Note, Image, Browser, Video, PDF, Anchor, Connector]
    ],
    area: Rectangle,
) -> List[
    Union[BaseWidget, Widget, Note, Image, Browser, Video, PDF, Anchor, Connector]
...
def find_widgets_containing_point(
    ...
    for widget in widgets:
        widget_rect = widget_bounding_box(widget)
        if (
            widget_rect.left <= point.x <= widget_rect.right
            and widget_rect.top <= point.y <= widget_rect.bottom
        ):
            result.append(widget)
    return result
