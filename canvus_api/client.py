...
class CanvusClient:
    ...
    def _classify_error(
        self, status_code: Optional[int], error_text: str
    ) -> CanvusAPIError:
        ...
        if status_code is None:
            return TimeoutError(
                f"Connection failed: {error_text}", status_code, error_text
            )
        elif status_code == 401:
            return AuthenticationError(
                f"Authentication failed: {error_text}", status_code, error_text
            )
        elif status_code == 404:
            return ResourceNotFoundError(
                f"Resource not found: {error_text}", status_code, error_text
            )
        elif status_code == 429:
            return RateLimitError(
                f"Rate limit exceeded: {error_text}", status_code, error_text
            )
        elif status_code == 408:
            return TimeoutError(
                f"Request timeout: {error_text}", status_code, error_text
            )
        elif 500 <= status_code < 600:
            return ServerError(
                f"Server error ({status_code}): {error_text}", status_code, error_text
            )
        else:
            return CanvusAPIError(
                f"API error ({status_code}): {error_text}", status_code, error_text
            )
    ...
    def _validate_response_against_request(
        ...
        for key, value in request_data.items():
            if key in response_data and response_data[key] != value:
                # Mismatch detected - could be enhanced with proper logging
                pass
                ...
                async with aiohttp.ClientSession(connector=connector) as session:
                    async with session.request(
                        ...
                        if status not in range(200, 300):
                            ...
                            if attempt < max_retries and self._is_retryable_error(
                                status, text
                            ):
    ...
    async def _stream_request(
        ...
        *,
        ...
        """Make a streaming request that yields raw lines.

        Unlike _request, this method properly keeps the connection open
        by using an async generator pattern.
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.request(method, url, **request_kwargs) as response:
                if response.status not in range(200, 300):
                    text = await response.text()
                    raise self._classify_error(response.status, text)

                # Read complete lines using readline() for proper NDJSON handling
                while True:
    ...
    async def subscribe(
        ...
        *,
        ...
        # Use _stream_request which properly keeps connection open
        async for line in self._stream_request("GET", endpoint, params=params):
            try:
                # Parse JSON data
    ...
    async def list_canvases(
        self, params: Optional[JsonData] = None, filter_obj: Optional[Filter] = None
    ) -> List[Canvas]:
        """
    ...
    async def list_widgets(
        self, canvas_id: str, filter_obj: Optional[Filter] = None
    ) -> List[Widget]:
        """
    ...
    async def subscribe_widgets(
        ...
        def parse_widget(
            data: Dict[str, Any],
        ) -> Union[Note, Image, Browser, Video, PDF, Widget]:
            """Parse a single widget dict into the appropriate model."""
            ...
            if widget_type == "note":
                ...
                return Browser.model_validate(data)
        ...
        async for line in self._stream_request(
            ...
            try:
                ...
                # Handle both array (initial state) and single object (updates)
                if isinstance(data, list):
                    # Initial state: array of all widgets
                    for item in data:
                        try:
                            result = parse_widget(item)
                            if callback:
                                callback(result)
                            yield result
                        except Exception:
                            # Skip widgets that fail to parse (e.g., Connectors)
                            continue
                else:
                    # Single widget update
                    result = parse_widget(data)
                    if callback:
                        callback(result)
                    yield result
    ...
    async def subscribe_note(
        ...
        # Use _stream_request which properly keeps connection open
        async for line in self._stream_request(
            "GET", f"canvases/{canvas_id}/notes/{note_id}", params=params
        ):
            try:
                # Parse JSON data
                data = json.loads(line)
